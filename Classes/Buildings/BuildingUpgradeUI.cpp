...
void BuildingUpgradeUI::setupUI()
{
    ...
    gemButton->addClickEventListener([this](Ref* sender) {
        ...
        if (_building->isMaxLevel()) return;
        // 检查工人
        ...
        if (hasRes && hasGem) {
            ...
            if (resMgr.consume(costType, cost)) {
                ...
                if (upgradeMgr->startUpgrade(_building, cost, 10.0f, true)) {
                    if (_resultCallback) _resultCallback(true, _building->getLevel());
                    onCloseClicked();
                }
                else {
                    // 失败回退
...
{
    if (!_building || _building->getBuildingType() != BuildingType::kArmy) return;
...
{
    switch (type) {
        case ResourceType::kGold: return "金币";
        case ResourceType::kElixir: return "圣水";
        case ResourceType::kGem: return "宝石";
        default: return "未知";
    }
}
