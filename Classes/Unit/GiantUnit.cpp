...
bool GiantUnit::init(int level) {
  ...
  // 播放部署音效
  AudioManager::GetInstance().PlayEffect(SoundEffectId::kGiantDeploy);
...
void GiantUnit::onAttackBefore() {
  // 播放攻击音效（随机变体）
  AudioManager::GetInstance().PlayEffect(SoundEffectId::kGiantAttack);
}

void GiantUnit::onAttackAfter() {
  // 播放命中音效（随机变体）
  AudioManager::GetInstance().PlayEffect(SoundEffectId::kGiantHit);
}

void GiantUnit::onDeathBefore() {
  // 播放死亡音效
  AudioManager::GetInstance().PlayEffect(SoundEffectId::kGiantDeath);
}